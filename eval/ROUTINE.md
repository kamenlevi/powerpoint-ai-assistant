...
`eval/run.js` evaluates the PowerPoint AI assistant against the test cases in `cases.json` (plus any auto-generated cases in `generated-cases.json`). For each case:

1. Constructs a prompt with the deck state, the user's request, and the current system prompt extracted from `server.js`.
...
- **New category**: When overall avg ≥95, a brand-new PowerPoint domain category gets 2 starter cases (≥90 promotes a level-2 category).
...
Budget cap: $0.50 per session (hard limit, raises an exception if exceeded). Typical cost: $0.01-0.05.
